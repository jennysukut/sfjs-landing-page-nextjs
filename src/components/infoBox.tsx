...
interface InfoBoxProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  addClasses?: string;
  size?: "small" | "standard" | "large" | "thin" | "extraLarge" | "extraSmall";
  ...
  const boxClasses = clsx(
    "InfoBox max-w-[95vw] relative z-[1] font-semibold leading-5 transition-all duration-200 tracking-superwide",
    {
      ...
      "py-8 px-8 xs:px-10 sm:py-8 sm:px-12 md:py-14 md:px-16 rounded-3xl":
        size === "large",
      "py-6 px-8 sm:py-8 sm:px-10 rounded-2xl sm:rounded-3xl": size === "thin",
      "py-8 px-8  w-[85vw] rounded-3xl min-h-[30rem]": size === "extraLarge",
      "py-3 px-6 sm:py-4 sm:px-8 rounded-2xl sm:rounded-[1.2rem]":
        size === "extraSmall",

      //width
      "max-w-screen-sm": width === "standard",
      "max-w-[85vw] ": width === "extraWide",
    },
    addClasses,
  );
